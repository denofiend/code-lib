...
{ "Success", "Operation failed", "Expected CRLF", "Job too big",
		"Queue draining", "Timed out", "Not found", "Deadline soon", "Buried",
		"Not ignored" };
...
{
	code = abs(code);
	return (code > sizeof(bs_status_verbose) / sizeof(char*)) ?
			0 : bs_status_verbose[code];
}
...
{
	char *token, *data;
	ssize_t bytes, data_size, status_size, status_max = 512, expect_data_bytes =
			0;
	...
	*token = 0;
	...
	data_size =
			message->size > BS_READ_CHUNK_SIZE ?
					message->size + BS_READ_CHUNK_SIZE : BS_READ_CHUNK_SIZE;
	...
	while (1)
	{
		...
		if (message->size >= expect_data_bytes)
			break;
	}
...
int bs_put(int fd, int priority, int delay, int ttr, char *data, size_t bytes)
{
	...
	BS_RETURN_FAIL_WHEN(message, bs_resp_expected_crlf,
			BS_STATUS_EXPECTED_CRLF);
...
int bs_reserve_job(int fd, char *command, BSJ **result)
{
	...
	if (BS_STATUS_IS(message->status, bs_resp_reserved))
	{
		*result = job = (BSJ*) malloc(sizeof(BSJ));
		...
		sscanf(message->status + strlen(bs_resp_reserved) + 1, "%d %lu",
				&job->id, &job->size);
	...
	BS_RETURN_FAIL_WHEN(message, bs_resp_deadline_soon,
			BS_STATUS_DEADLINE_SOON);
	BS_RETURN_INVALID(message);
}
...
int bs_peek_job(int fd, char *command, BSJ **result)
{
	...
	if (BS_STATUS_IS(message->status, bs_resp_found))
	{
		*result = job = (BSJ*) malloc(sizeof(BSJ));
		...
		sscanf(message->status + strlen(bs_resp_found) + 1, "%d %lu", &job->id,
				&job->size);
		job->data = message->data;
		message->data = 0;
		bs_free_message(message);
...
int bs_list_tube_used(int fd, char **tube)
{
	...
	{
		*tube = (char*) calloc(1,
				strlen(message->status) - strlen(bs_resp_using) + 1);
