...
#include "beanstalk_client/beanstalk.h"
#include <string>
#include <vector>
#include <map>
...
public:
	~BeanstalkClient();
	BeanstalkClient(std::string host, int port);
	bool ping();
	bool use(std::string);
	bool watch(std::string);
	bool ignore(std::string);
	int put(std::string data, int priority = 0, int delay = 0, int ttr = 60);
	int put(char *data, size_t bytes, int priority = 0, int delay = 0, int ttr =
			60);
	bool del(int id);
	bool reserve(Job &);
